...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    const INT MoveExhaust = 100 MILLISECONDS;//BLBLBL 200=>400=>200 remis à 200 sinon on a un accoup au démarrage d'une séquence de marche // steph ajout de INT // steph 100 au lieu de 200
...
//		if( user->self->ViewFlag( __FLAG_BERSERK ) == 0 ){
			...
			WorldPos saved_pos = user->self->GetWL();
					...
					if( newExhaust.move <= TFCMAIN::GetRound() /*|| ( newExhaust.boWalking && newExhaust.move - TFCMAIN::GetRound() <= MoveExhaust )*/ ){//BLBL à mon avis c'est ptet cette condition qui laisse passer trop de paquets de mouvements.
						...
						if (user->MoveList.size()<3 &&  newExhaust.move < TFCMAIN::GetRound() + 100 MILLISECONDS) { // steph 100 au lieu de 200
